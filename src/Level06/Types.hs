...
portFromInt :: Int -> Port
portFromInt = Port . fromIntegral
...
partialConfDecoder = D.withCursor $ \curs -> do
  io <- D.down curs
  PartialConf <$> D.fromKey "port" ((Last . Just . Port . fromIntegral) <$> D.int) io
              <*> D.fromKey "dbName" ((Last . Just . DBFilePath) <$> D.string) io
