...
main :: IO ()
main = do
  let conf = Conf { port = Port 3000
                  , dbFilePath = DBFilePath ":memory:"
                  }
  edb <- DB.initDB (DBFilePath ":memory:")
  case edb of
    Left e -> failInit e
    Right db -> do
      let env = Env { envLoggingFn = liftIO . hPutStrLn stderr
                    , envConfig    = conf
                    , envDB        = db
                    }
      runTests env
...
runTests :: Env -> IO ()
runTests env = do
  defaultMain $ testGroup "Applied FP Course - Tests 07"
    [ testWai (Core.app env) "Successful post" $ do
        post "fudge/add" "Comment!" >>= assertStatus' HTTP.status200

    , testWai (Core.app env) "List topics" $ do
        topics <- get "list"
        assertStatus' HTTP.status200 topics
        assertBody "[\"fudge\"]" topics

    , testWai (Core.app env) "View Comments" $ do
    ...
    , testWai (Core.app env) "Topic not stated" $ do
        resp <- post "/add" "Comment"
        assertStatus' HTTP.status404 resp
        assertBody "Unknown Route" resp

    , testWai (Core.app env) "Empty request" $ do
        resp <- post "" ""
        assertStatus' HTTP.status404 resp
        assertBody "Unknown Route" resp

    , testWai (Core.app env) "Invalid action" $ do
